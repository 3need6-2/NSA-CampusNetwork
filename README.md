...
多格式导入：支持 CSV、PCAP、PCAPNG、CAP 文件，统一转换为标准化流量元数据后分析
...
实时网卡采集：从本机选定网卡采集流量，可设置 BPF 过滤条件和自动停止时长
活动识别：展示可识别的 DNS/HTTP/TLS 站点访问、HTTP 下载文件和本机网络进程关联
证据留存：每次实时采集同时保存原始 PCAP 与元数据 CSV，默认保留 3 天并支持手动清理
...
- **Multi-format Import** - Import CSV, PCAP, PCAPNG, and CAP files into one canonical analysis schema
...
- **Live Interface Capture** - Capture from a selected local interface with optional BPF filtering, duration limits, and PCAP evidence output
- **Activity Discovery** - Summarize recognized DNS/HTTP/TLS destinations, HTTP downloads, and locally correlated network processes
- **Evidence Retention** - Store a PCAP and metadata CSV for each capture session; retention defaults to 3 days
...
| View | Description |
|------|-------------|
| Home Console | System status cards, CSV/PCAP upload, navigation |
...
| Live Capture | Interface selection, evidence retention, recent packets, and recognized activity |
...
│   ├── ingestion.py             # CSV/PCAP import, live capture, and evidence retention
...
│   ├── live.html               # Live interface capture page
...
│   ├── live_traffic.csv        # Metadata generated by latest live capture
│   ├── uploads/                # Imported source files
│   ├── captures/               # Per-session PCAP and metadata CSV evidence
...
- Python 3.10 至 3.12（推荐；项目依赖兼容 Python 3.12）
- pip 包管理工具
- 现代浏览器（Chrome、Edge、Firefox 等）
- 网络连接：前端通过 CDN 加载 Chart.js
- 实时抓包（Windows）：安装 [Npcap](https://npcap.com/) 后再启动应用；仅分析本机有权限采集的网卡流量
...
- **导入离线流量**：上传 CSV、PCAP、PCAPNG 或 CAP 文件；系统会转换为 `data/traffic.csv` 并刷新分析结果
- **查看安全仪表板**：点击“进入安全仪表板”进入 `/dashboard`
- **进入实时大屏**：点击“实时态势大屏”进入 `/realtime`
- **实时抓包与证据保存**：点击“原始抓包 / 证据保存”进入 `/live`
...
upload:
  allowed_extensions: "csv,pcap,pcapng,cap"  # Supported import formats
  max_size_mb: 250                            # Runtime upload limit (MB)
...
支持以下文件类型，单次上传上限为 **250 MB**：

- **CSV**：流量元数据表；系统可识别常见列别名，例如 `src_ip/source_ip`、`dst_ip/destination_ip`、`bytes/length/size`。
- **PCAP / PCAPNG / CAP**：通过 Scapy 提取 IP、端口、协议、长度和可识别的活动元数据。

CSV 至少应包含源和目的地址字段。建议使用以下标准列（用逗号分隔）：
...
- `data/uploads/`：保留上传的原始文件，便于追溯导入来源

### 可识别的活动信息

导入抓包或进行实时采集时，系统会尽力从可见流量中提取：

- DNS 查询域名；
- 明文 HTTP 的 `Host`、请求路径与 `Content-Disposition` 下载文件名；
- TLS ClientHello 中可用的 SNI 站点名称；
- 本机当前连接表中可关联到的进程名称和 PID。

HTTPS 内容、QUIC 流量、应用私有协议和未出现在本机连接表中的流量通常只能显示协议、IP、端口或域名线索，页面内容和应用名称不保证始终可见。
...
| `/live`                     | GET  | 实时网卡抓包、活动识别和证据保存页面                         |
| `/upload`                   | POST | 导入 CSV、PCAP、PCAPNG 或 CAP 文件并自动刷新分析             |
| `/api/stats`                | GET  | 返回 JSON 格式基础统计数据                                   |
| `/api/activities`           | GET  | 返回域名、进程、下载和活动汇总                               |
...
| `/api/live/interfaces`      | GET  | 返回可选本机抓包网卡                                         |
| `/api/live/status`          | GET  | 返回实时抓包状态、最近数据包和活动汇总                       |
| `/api/live/start`           | POST | 启动实时网卡抓包与证据写入                                   |
| `/api/live/stop`            | POST | 停止抓包，将本次元数据载入仪表板分析                         |
| `/api/evidence/config`      | GET  | 返回证据目录、保留天数和现有证据文件                         |
| `/api/evidence/cleanup`     | POST | 按指定保留天数删除过期 `capture_*` 证据文件                  |
...
当启动 Flask 应用或导入新的流量文件时，系统会自动生成用户画像数据。
...
实时采集页面位于 `/live`，由 `utils/ingestion.py` 中的 `LiveCaptureService` 管理单个采集会话。与 `/realtime` 的 CSV 回放不同，`/live` 会从选定本机网卡读取实际数据包。

使用流程：

1. 在 Windows 上先安装 Npcap，然后启动应用并访问 `http://localhost:5001/live`。
2. 选择 Wi-Fi、以太网或热点对应网卡；可选填 BPF 过滤条件，例如 `tcp` 或 `udp port 53`。
3. 选择手动停止或 5 分钟至 2 小时的自动停止时长，并设置证据保存目录。
4. 选择自动删除周期（默认 3 天；可选 1 至 365 天），点击“开始采集”。
5. 页面会持续显示最近数据包、协议分类、域名/站点、HTTP 下载线索及可关联的本机网络进程。
6. 点击“停止并分析”后，系统关闭证据文件并把本次元数据载入仪表板；可继续在 `/dashboard` 查看统计、画像和安全审查。

每次会话在证据目录（默认 `data/captures/`）生成两个同名会话文件：

```text
capture_YYYYMMDD_HHMMSS_<id>.pcap
capture_YYYYMMDD_HHMMSS_<id>_metadata.csv
```

- PCAP 保存实际捕获的原始数据包，适合后续复核；
- CSV 保存标准化元数据和已识别活动，适合快速分析；
- 清理逻辑仅删除证据目录中以 `capture_` 开头且已超过保留期的 `.pcap`、`.pcapng`、`.cap`、`.csv` 文件；
- 每次启动采集和读取状态时都会执行一次过期清理，也可以调用清理接口手动执行。

启动和停止采集示例：

```bash
# 查看网卡
curl http://localhost:5001/api/live/interfaces

# 启动采集：使用默认网卡，保留 15 分钟，证据保留 3 天
curl -X POST http://localhost:5001/api/live/start ^
  -H "Content-Type: application/json" ^
  -d "{\"interface\":\"\",\"filter\":\"\",\"duration_seconds\":900,\"retention_days\":3,\"evidence_dir\":\"data/captures\"}"

# 查看实时状态
curl http://localhost:5001/api/live/status

# 停止并载入本次采集结果
curl -X POST http://localhost:5001/api/live/stop

# 按 7 天保留期手动清理已过期的证据
curl -X POST http://localhost:5001/api/evidence/cleanup ^
  -H "Content-Type: application/json" ^
  -d "{\"retention_days\":7}"
```

> 原始 PCAP 可能包含敏感网络数据。请把证据目录放在受控存储中，按实际需要设置保留天数，并在共享或导出前做好脱敏。
...
- 展示系统状态、当前数据统计和 CSV/PCAP 上传入口
- 有数据时显示“进入安全仪表板”和“实时态势大屏”入口
- 提供“原始抓包 / 证据保存”入口
- 上传新流量文件后自动重新分析并跳转到仪表板
...
- 读取本机网卡列表，支持选择网卡与 BPF 过滤条件
- 支持手动停止或按设置时长自动停止，并实时展示包数、字节数、IP 数和采集时长
- 保存原始 PCAP 与分析元数据 CSV，默认保存到 `data/captures/`
- 显示可识别的 DNS/HTTP/TLS 站点、HTTP 下载和本机进程关联
- 可选择 1 至 365 天的证据自动删除时间，默认 3 天

限制和注意事项

- 文件大小限制：最大文件大小 250MB
- 离线导入格式：CSV、PCAP、PCAPNG、CAP
- 时间戳格式：建议使用 `YYYY-MM-DD HH:MM:SS`
- 导入后的标准化数据保存为 `data/traffic.csv`；原始上传文件保留在 `data/uploads/`
- Windows 实时抓包依赖 Npcap 和可用网卡；热点测试时请选择承载热点流量的 Wi-Fi 网卡
- 原始 PCAP 会保留包内容，证据目录需要按敏感数据管理
- 站点、下载和进程识别依赖可见协议字段及本机连接表；加密或应用私有流量只能提供有限线索
...
- 检查扩展名是否为 CSV、PCAP、PCAPNG 或 CAP
- CSV 至少应存在可映射的源 IP 与目的 IP 列；建议使用标准列名
- 验证 `timestamp` 能被 Pandas 解析为时间，`bytes`、`src_port`、`dst_port` 为数字
- 导入抓包文件时，确认已执行 `python -m pip install -r requirements.txt` 以安装 Scapy

问题：图表不显示

解决方案：

- 检查是否已导入有效的流量文件（CSV、PCAP、PCAPNG 或 CAP）
- 查看浏览器控制台是否有错误信息
...
- Windows 上安装 Npcap 后重新打开终端和应用；通过 `python -c "from scapy.all import get_if_list; print(get_if_list())"` 检查网卡可见性
- 在 `/live` 点击“刷新网卡”，选择实际承载网络流量的 Wi-Fi、以太网或热点网卡
- 先清空过滤条件进行测试；BPF 过滤条件错误会导致采集启动失败或结果为空
- 检查 `data/captures/` 或页面所设证据目录是否可写
- 访问 `/api/live/status` 查看服务返回的最近错误信息
...
|---|---|
| Backend Framework | Flask 3.x, Werkzeug 3.x |
| Data Processing | Pandas 2.2+, NumPy 1.26+ |
| Packet Processing | Scapy 2.5+, psutil 6+ |
...
- 实时抓包页面样式：编辑 `templates/live.html`
- 实时大屏样式：编辑 `templates/realtime.html`

性能优化建议

1. **大文件处理**：对超大 CSV 和抓包文件使用分块或流式解析，避免一次性加载占用过多内存
...
2. **文件上传**：验证 CSV 与抓包文件的类型、内容、大小、列名和行数，避免异常文件拖垮服务
